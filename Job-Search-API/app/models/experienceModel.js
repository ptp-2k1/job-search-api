...
module.exports = function() {
    this.getAll = async function(param, result) {
        ...
        const query = "SELECT Id, Name, CreateDate FROM [Experience] ORDER BY Name";
    ...
    this.filter = async function(param, result) {
        ...
        const pool = await conn;
        const query = "SELECT Id, Name, CreateDate FROM [Experience] WHERE Name LIKE N'%" + param.info +
    ...
    this.checkDuplicateName = async function(param, result) {
        const pool = await conn;
        const query = "IF EXISTS(SELECT * FROM [Experience] WHERE Name=@name AND Id = @id) SELECT 1 AS Result " +
            "ELSE IF EXISTS(SELECT * FROM [Experience] WHERE Name=@name) SELECT 0 AS Result ELSE SELECT 1" +
            "AS Result";
    ...
    this.create = async function(param, result) {
        const pool = await conn;
        const query = "INSERT INTO [Experience] (Name, CreateDate, UserId) VALUES (@name, CURRENT_TIMESTAMP, @userId)";
        return await pool.request()
            .input("name", sql.NVarChar, param.name)
    ...
    this.getDetail = async function(param, result) {
        const pool = await conn;
        const query = "SELECT Name FROM [Experience] WHERE Id = @id";
    ...
    this.update = async function(param, result) {
        const pool = await conn;
        const query = "UPDATE [Experience] SET Name=@name WHERE Id=@id";
        return await pool.request()
            .input("name", sql.NVarChar, param.name)
            .input("id", sql.Int, param.id)
            .query(query, function(err, data) {
    ...
    this.checkDelete = async function(param, result) {
        const pool = await conn;
        const query = "IF EXISTS(SELECT * FROM [JobProfile] WHERE ExperienceId=@id) SELECT 1 AS Result " +
            "ELSE IF EXISTS(SELECT * FROM [Post] WHERE ExperienceId=@id) SELECT 1 AS Result " +
    ...
    this.delete = async function(param, result) {
        const pool = await conn;
        const query = "DELETE FROM [Experience] WHERE Id = @id";
