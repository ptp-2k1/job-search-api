...
module.exports = function() {
    this.getAll = async function(param, result) {
        ...
        const query = "SELECT Id, Name, CreateDate FROM [Title] ORDER BY Name";
    ...
    this.filter = async function(param, result) {
        ...
        const pool = await conn;
        const query = "SELECT Id, Name, CreateDate FROM [Title] WHERE Name LIKE N'%" + param.info +
    ...
    this.checkDuplicateName = async function(param, result) {
        const pool = await conn;
        const query = "IF EXISTS(SELECT * FROM [Title] WHERE Name=@name AND Id = @id) SELECT 1 AS Result " +
            "ELSE IF EXISTS(SELECT * FROM [Title] WHERE Name=@name) SELECT 0 AS Result ELSE SELECT 1" +
            "AS Result";
    ...
    this.create = async function(param, result) {
        const pool = await conn;
        const query = "INSERT INTO [Title] (Name, CreateDate, UserId) VALUES (@name, CURRENT_TIMESTAMP, @userId)";
        return await pool.request()
            .input("name", sql.NVarChar, param.name)
    ...
    this.getDetail = async function(param, result) {
        const pool = await conn;
        const query = "SELECT Name FROM [Title] WHERE Id = @id";
    ...
    this.update = async function(param, result) {
        const pool = await conn;
        const query = "UPDATE [Title] SET Name=@name WHERE Id=@id";
        return await pool.request()
            .input("name", sql.NVarChar, param.name)
            .input("id", sql.Int, param.id)
            .query(query, function(err, data) {
    ...
    this.checkDelete = async function(param, result) {
        const pool = await conn;
        const query = "IF EXISTS(SELECT * FROM [JobProfile] WHERE TitleId=@id) SELECT 1 AS Result " +
            "ELSE IF EXISTS(SELECT * FROM [Post] WHERE TitleId=@id) SELECT 1 AS Result " +
