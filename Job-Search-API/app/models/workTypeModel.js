...
module.exports = function() {
    this.getAll = async function(param, result) {
        ...
        const query = "SELECT Id, Name, CreateDate FROM [WorkType] ORDER BY Name";
    ...
    this.filter = async function(param, result) {
        ...
        const pool = await conn;
        const query = "SELECT Id, Name, CreateDate FROM [WorkType] WHERE Name LIKE N'%" + param.info +
    ...
    this.checkDuplicateName = async function(param, result) {
        const pool = await conn;
        const query = "IF EXISTS(SELECT * FROM [WorkType] WHERE Name=@name AND Id = @id) SELECT 1 " +
            "AS Result ELSE IF EXISTS(SELECT * FROM [WorkType] WHERE Name=@name) SELECT 0 AS Result " +
    ...
    this.create = async function(param, result) {
        const pool = await conn;
        const query = "INSERT INTO [WorkType] (Name, CreateDate, UserId) VALUES (@name, CURRENT_TIMESTAMP, @userId)";
        return await pool.request()
            .input("name", sql.NVarChar, param.name)
    ...
    this.getDetail = async function(param, result) {
        const pool = await conn;
        const query = "SELECT Name FROM [WorkType] WHERE Id = @id";
    ...
    this.update = async function(param, result) {
        const pool = await conn;
        const query = "UPDATE [WorkType] SET Name=@name WHERE Id=@id";
        return await pool.request()
            .input("name", sql.NVarChar, param.name)
            .input("id", sql.Int, param.id)
            .query(query, function(err, data) {
    ...
    this.checkDelete = async function(param, result) {
        const pool = await conn;
        const query = "IF EXISTS(SELECT * FROM [JobProfile] WHERE WorkTypeId=@id) SELECT 1 AS Result " +
            "ELSE IF EXISTS(SELECT * FROM [Post] WHERE WorkTypeId=@id) SELECT 1 AS Result " +
    ...
    this.delete = async function(param, result) {
        const pool = await conn;
        const query = "DELETE FROM [WorkType] WHERE Id = @id";
