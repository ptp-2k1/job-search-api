...
import { filterArea, checkAreaDelete, deleteArea } from "../../../service/AreaService";
...
const AreaList = () => {
    ...
    const handleDelete = async (id) => {
        const result = await checkAreaDelete({
            id: id
        })

        if(result && result.status === 200) {
            Modal.confirm({
                title: "Chắc chắn xóa khu vực này ?",
                ...
                async onOk() {
                    if(!result.data.Result) {
                        ...
                        if(result && result.status === 200) {
                            ...
                            toast.error("Xóa khu vực thất bại ! Hãy thử lại sau", {
                                position: toast.POSITION.TOP_RIGHT
                            });
                        }
                    } else {
                        toast.warn("Không thể xóa vì khu vực này đã được áp dụng", {
                            position: toast.POSITION.TOP_RIGHT
                        });
                    }
                },
    ...
    const getData = async () => {
        const areaList = await filterArea({
            info: search,
        ...
        if(areaList && areaList.status === 200) {
            if(areaList.data) {
                setArea(areaList.data.slice((currentPage-1)* PAGINATION.Row, currentPage * PAGINATION.Row))
                setPageCount(Math.ceil(areaList.data.length / PAGINATION.Row))
            } else {
                setArea([])
                setPageCount(0)
            }
        } else {
            toast.error("Không thể lấy danh sách khu vực ngay lúc này ! Hãy thử lại sau", {
                position: toast.POSITION.TOP_RIGHT
            })
        }
    }
                ...
                <div className="col-12 grid-margin">
                    <div className="card">
                        <div className="card-body">
                            <h4 className="card-title">Danh sách khu vực</h4>
                            <Input.Search
                                className='mt-3 mb-3'
                                ...
                                >
                                    <thead style={{ position: "sticky", top: "0", borderColor: "black" }}>
                                        <tr style={{ textAlign: "center", backgroundColor: "#afcbdb" }}>
                                            ...
                                            <th style={{ border: "1px solid black"}} >Tên khu vực</th>
