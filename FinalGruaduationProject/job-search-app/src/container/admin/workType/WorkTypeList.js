...
import { filterWorkType, checkWorkTypeDelete, deleteWorkType } from "../../../service/WorkTypeService";
...
const WorkTypeList = () => {
    ...
    const handleDelete = async (id) => {
        const result = await checkWorkTypeDelete({
            id: id
        })

        if(result && result.status === 200) {
            Modal.confirm({
                title: "Chắc chắn xóa loại hình công việc này ?",
                icon: <InfoCircleTwoTone />,
                async onOk() {
                    if(!result.data.Result) {
                        const result = await deleteWorkType(id)
                        ...
                        if(result && result.status === 200) {
                            ...
                            toast.error("Xóa loại hình công việc thất bại ! Hãy thử lại sau", {
                                position: toast.POSITION.TOP_RIGHT
                            });
                        }
                    } else {
                        toast.warn("Không thể xóa vì loại hình công việc này đã được áp dụng", {
                            position: toast.POSITION.TOP_RIGHT
                        });
                    }
                },
    ...
    const getData = async () => {
        const workTypeList = await filterWorkType({
            info: search,
        ...
        if(workTypeList && workTypeList.status === 200) {
            if(workTypeList.data) {
                setWorkType(workTypeList.data.slice((currentPage-1)* PAGINATION.Row, currentPage * PAGINATION.Row))
                setPageCount(Math.ceil(workTypeList.data.length / PAGINATION.Row))
            } else {
                setWorkType([])
                setPageCount(0)
            }
        } else {
            toast.error("Không thể lấy danh sách loại hình công việc ngay lúc này ! Hãy thử lại sau", {
                position: toast.POSITION.TOP_RIGHT
            })
        }
    }
                ...
                <div className="col-12 grid-margin">
                    <div className="card">
                        <div className="card-body">
                            <h4 className="card-title">Danh sách loại hình công việc</h4>
                            <Input.Search
                            ...
                            <div className="table-responsive pt-2 mb-3">
                                ...
                                >
                                    <thead style={{ position: "sticky", top: "0", borderColor: "black" }}>
                                        <tr style={{ textAlign: "center", backgroundColor: "#afcbdb" }}>
                                            ...
                                            <th style={{ border: "1px solid black"}} >Tên loại hình công việc</th>
                                                    ...
                                                    <td>
                                                        <div className="text-center">
                                                            <a
                                                                ...
                                                                href={`work-type-management/${item.Id}`}
                                                            >
                                ...
                                {
                                    (!workType || workType && workType.length === 0) &&
                                    (
