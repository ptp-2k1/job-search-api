...
import { filterExperience, checkExperienceDelete, deleteExperience } from "../../../service/ExperienceService";
...
const ExperienceList = () => {
    ...
    const handleDelete = async (id) => {
        const result = await checkExperienceDelete({
            id: id
        })

        if(result && result.status === 200) {
            Modal.confirm({
                title: "Chắc chắn xóa kinh nghiệm này ?",
                ...
                async onOk() {
                    if(!result.data.Result) {
                        ...
                        if(result && result.status === 200) {
                            ...
                            toast.error("Xóa kinh nghiệm thất bại ! Hãy thử lại sau", {
                                position: toast.POSITION.TOP_RIGHT
                            });
                        }
                    } else {
                        toast.warn("Không thể xóa vì kinh nghiệm này đã được áp dụng", {
                            position: toast.POSITION.TOP_RIGHT
                        });
                    }
                },
    ...
    const getData = async () => {
        const experienceList = await filterExperience({
            info: search,
        ...
        if(experienceList && experienceList.status === 200) {
            if(experienceList.data) {
                setExperience(experienceList.data.slice((currentPage-1)* PAGINATION.Row, currentPage * PAGINATION.Row))
                setPageCount(Math.ceil(experienceList.data.length / PAGINATION.Row))
            } else {
                setExperience([])
                setPageCount(0)
            }
        } else {
            toast.error("Không thể lấy danh sách kinh nghiệm ngay lúc này ! Hãy thử lại sau", {
                position: toast.POSITION.TOP_RIGHT
            })
        }
    }
                            ...
                            >
                                <thead style={{ position: "sticky", top: "0", borderColor: "black" }}>
                                    <tr style={{ textAlign: "center", backgroundColor: "#afcbdb" }}>
                                        ...
                                        <th style={{ border: "1px solid black"}} >Tên kinh nghiệm</th>
                                                ...
                                                <td>
                                                    <div className="text-center">
                                                        <a
                                                            ...
                                                            href={`experience-management/${item.Id}`}
                                                        >
