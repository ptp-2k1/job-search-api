...
import { filterSalary, checkSalaryDelete, deleteSalary } from "../../../service/SalaryService";
...
const SalaryList = () => {
    ...
    const handleDelete = async (id) => {
        const result = await checkSalaryDelete({
            id: id
        })

        if(result && result.status === 200) {
            Modal.confirm({
                title: "Chắc chắn xóa mức lương này ?",
                ...
                async onOk() {
                    if(!result.data.Result) {
                        ...
                        if(result && result.status === 200) {
                            toast.success("Xóa mức lương thành công", {
                                position: toast.POSITION.TOP_RIGHT
                            });
                            setCurrentPage(1)
                            setChange(!change)
                        } else {
                            toast.error("Xóa mức lương thất bại ! Hãy thử lại sau", {
                                position: toast.POSITION.TOP_RIGHT
                            });
                        }
                    } else {
                        toast.warn("Không thể xóa vì mức lương này đã được áp dụng", {
                            position: toast.POSITION.TOP_RIGHT
                        });
                    }
                },
    ...
    const getData = async () => {
        const salaryList = await filterSalary({
            info: search,
        ...
        if(salaryList && salaryList.status === 200) {
            if(salaryList.data) {
                setSalary(salaryList.data.slice((currentPage-1)* PAGINATION.Row, currentPage * PAGINATION.Row))
                setPageCount(Math.ceil(salaryList.data.length / PAGINATION.Row))
            } else {
                setSalary([])
                setPageCount(0)
            }
        } else {
            toast.error("Không thể lấy danh sách mức lương ngay lúc này ! Hãy thử lại sau", {
                position: toast.POSITION.TOP_RIGHT
            })
        }
    }
            ...
            <div className="col-12 grid-margin">
                <div className="card">
                    <div className="card-body">
                        <h4 className="card-title">Danh sách mức lương</h4>
                        <Input.Search
                            className='mt-3 mb-3'
                            ...
                            >
                                <thead style={{ position: "sticky", top: "0", borderColor: "black" }}>
                                    <tr style={{ textAlign: "center", backgroundColor: "#afcbdb" }}>
                                        ...
                                        <th style={{ border: "1px solid black"}} >Tên mức lương</th>
                                                ...
                                                <td>
                                                    <div className="text-center">
                                                        <a
                                                            ...
                                                            href={`salary-management/${item.Id}`}
                                                        >
