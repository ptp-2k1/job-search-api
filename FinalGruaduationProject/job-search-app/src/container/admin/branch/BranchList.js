...
import { filterBranch, checkBranchDelete, deleteBranch } from "../../../service/BranchService";
...
const BranchList = () => {
    ...
    const handleDelete = async (id) => {
        const result = await checkBranchDelete({
            id: id
        })

        if(result && result.status === 200) {
            Modal.confirm({
                title: "Chắc chắn xóa ngành nghề này ?",
                ...
                async onOk() {
                    if(!result.data.Result) {
                        ...
                        if(result && result.status === 200) {
                            ...
                            toast.error("Xóa ngành nghề thất bại ! Hãy thử lại sau", {
                                position: toast.POSITION.TOP_RIGHT
                            });
                        }
                    } else {
                        toast.warn("Không thể xóa vì ngành nghề này đã được áp dụng", {
                            position: toast.POSITION.TOP_RIGHT
                        });
                    }
                },
    ...
    const getData = async () => {
        const branchList = await filterBranch({
            info: search,
        ...
        if(branchList && branchList.status === 200) {
            if(branchList.data) {
                setBranch(branchList.data.slice((currentPage-1)* PAGINATION.Row, currentPage * PAGINATION.Row))
                setPageCount(Math.ceil(branchList.data.length / PAGINATION.Row))
            } else {
                setBranch([])
                setPageCount(0)
            }
        } else {
            toast.error("Không thể lấy danh sách ngành nghề ngay lúc này ! Hãy thử lại sau", {
                position: toast.POSITION.TOP_RIGHT
            })
        }
    }
            ...
            <div className="col-12 grid-margin">
                <div className="card">
                    <div className="card-body">
                        <h4 className="card-title">Danh sách ngành nghề</h4>
                        <Input.Search
                            className='mt-3 mb-3'
                            ...
                            >
                                <thead style={{ position: "sticky", top: "0", borderColor: "black" }}>
                                    <tr style={{ textAlign: "center", backgroundColor: "#afcbdb" }}>
                                        ...
                                        <th style={{ border: "1px solid black"}} >Tên ngành nghề</th>
                                                ...
                                                <td>
                                                    <div className="text-center">
                                                        <a
                                                            ...
                                                            href={`branch-management/${item.Id}`}
                                                        >
