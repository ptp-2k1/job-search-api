...
const AdminChangePassword = () => {
    ...
    let handleChangePassword = async () => {
        ...
        if(result && result.status === 200) {
            if(result.data) {
                ...
                if(result && result.status === 200) {
                    if (localStorage.getItem("changePasswordRole") === String(Role.Admin)) {
    ...
    return (
        <>
            <div style={{ height: "1px", backgroundColor: "#eaedff" }} />
            <div className="col-12 grid-margin mt-70" style={{backgroundColor: "#eaedff"}}>
